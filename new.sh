...
configure_external_access() {
    log "Настройка доступа из внешней сети..."
...
EOF
    ...
    ufw allow from any to any port $APP_PORT
    
    log "Внешний доступ настроен"
    echo "IP адрес VM: $VM_IP"
    echo "Доступ из Windows: http://$VM_IP/api/health"
}
...
show_deployment_info() {
    ...
    echo
    echo "=============================================="
    echo "    ДОСТУП ИЗ WINDOWS БРАУЗЕРА"
    echo "=============================================="
    echo "IP адрес VM:        $VM_IP"
    echo "Health Check:       http://$VM_IP/api/health"
    echo "Products API:       http://$VM_IP/api/products"
    echo "Users API:          http://$VM_IP/api/users"
    echo "Detailed Health:    http://$VM_IP/api/health/detailed"
    echo
    echo "=============================================="
    echo "    ТЕСТИРОВАНИЕ ИЗ WINDOWS"
    echo "=============================================="
    echo "1. Откройте браузер в Windows"
    echo "2. Перейдите по адресу: http://$VM_IP/api/health"
    echo "3. Вы должны увидеть JSON ответ:"
    echo '   {"status":"UP","timestamp":"...","service":"Simple Web App"}'
    echo
    echo "4. Протестируйте API endpoints:"
    echo "   - http://$VM_IP/api/products"
    echo "   - http://$VM_IP/api/users"
    echo
    echo "=============================================="
    echo "    CURL КОМАНДЫ ДЛЯ WINDOWS PowerShell"
    echo "=============================================="
    echo "Invoke-RestMethod -Uri http://$VM_IP/api/health"
    echo "Invoke-RestMethod -Uri http://$VM_IP/api/products"
    echo "Invoke-RestMethod -Uri http://$VM_IP/api/users"
}

show_deployment_info_with_external_access() {
    show_deployment_info
}
